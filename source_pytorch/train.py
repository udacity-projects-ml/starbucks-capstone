...
def _get_train_data_loader(batch_size, training_dir):
    ...
    print('batch_size: ', training_dir)

    train_data = pd.read_csv(os.path.join(training_dir, "train.csv"), header=None, names=None)
    
    print('train_data length: ', train_data.shape[0])
    print('train_data number of cols: ', train_data.shape[1])
    
    print(train_data.head(10))
    
    tmp_y = train_data[[0]]
    tmp_x = train_data.drop([0], axis=1)
    
    #print('Number of positive training examples: ', pos_examples.shape[0])
    #print('Positive examples - number of cols: ', pos_examples.shape[1])
    
    print(tmp_y.head(500))
    print(tmp_x.head(500))
    
    print('Number of positive labels: ', train_data[[0]].values.sum())
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            for idx, pred in enumerate(y_pred):
                if pred > 0.5:
                    #print(batch_x[idx], ' has pred ', pred)
                    tmp_pos_preds = tmp_pos_preds + 1
        ...
        print('Positive predicted in epoch ', epoch, ': ', tmp_pos_preds)
